$(document).ready(function() {
	...
	var $local = {
		...
		$tablaReporteIngresosDetalle : $("#tablaReporteIngresosDetalle2"),
		tablaReporteIngresosGeneral : "",
		$tablaReporteIngresosGeneral : $("#tablaReporteIngresosGeneral2"),
		$limpiar : $("#limpiar2"),
		$buscar : $("#buscar2"),
		$exportar : $("#exportar2") 
	}

	$formReporteIngresosAportantes = $("#formReporteIngresosAportantes");
	...
	$local.$buscar.on("click", function() {
		...
		if (!$formReporteIngresosAportantes.valid()) {
			return;
		}
		/*
		...
		var criterioBusqueda = $formReporteIngresosAportantes.serializeJSON();
		console.log(criterioBusqueda.nroDocCliente);
		
		criterioBusqueda.verbo = "DET_APO";
		$.ajax({
			type : "GET",
			url : $variableUtil.root + "ingresos/reporte?accion=buscar4",
...
/*
	...
	$local.$limpiar.on("click", function() {
		var criterioBusqueda = $formReporteIngresosAportantes.serializeJSON();
		if ($funcionUtil.camposVacios($formReporteIngresosAportantes)) {
		...
		criterioBusqueda.verbo = "DET_LIMPIAR_APO";
		$.ajax({
			type : "GET",
			url : $variableUtil.root + "ingresos/reporte?accion=buscar4",
			data : criterioBusqueda,
